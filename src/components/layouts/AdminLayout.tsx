...
const Sidebar = () => {
  ...
  const handleLogout = () => {
    ...
    if (confirmLogout) {
      ...
      localStorage.removeItem('authToken'); // Hapus JWT token jika ada
      navigate('/login');
    }
  };
  ...
  return (
    <aside className="w-64 bg-brand-dark text-gray-200 p-6 flex flex-col h-screen overflow-y-auto">
      ...
      <nav className="flex-grow">
        <ul className="space-y-2">
          ...
          {/* PENGATURAN & LOGOUT - Tidak pakai mt-auto */}
          ...
          <li>
            <button onClick={handleLogout} className="w-full flex items-center gap-3 py-2 px-4 text-red-400 hover:bg-red-900/50 rounded-md transition-colors">
...
const AdminLayout = () => {
  return (
    <div className="flex min-h-screen"> 
      <Sidebar />
      <main className="flex-grow p-8 bg-gray-100 overflow-y-auto"><Outlet /></main>
    </div>
  );
};
